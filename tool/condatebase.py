...
class data_processing():
    ...
    def select_data(table_name,database_name,select_table,select_line,select_condition):
    ...
    # 添加数据 sql_line（添加的字段名）
    def insert_data(self,database_name,insert_table,sql_line,insert_value):
        sql_line=tuple(sql_line).__str__().replace("\'", "") # 获取表格的字段名，并将列表转化元组转化为为字符串
        line_value=tuple(insert_value).__str__() # 将传入的值列表转化为字符串
        sql="insert into %s %s  values %s" % (insert_table,sql_line,line_value)
        cur.execute(sql)
        conn.commit()
        return cur.rowcount # =0 失败 >0 成功
